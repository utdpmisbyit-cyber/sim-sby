                        ...
                        @foreach($columns as $column)
                            <div class="col-md-4">
                                @foreach($column as $question)
                                    ...
                                    <div class="form-group mb-4 d-flex flex-column align-items-start" id="qgroup_{{ $question['name'] }}">
                                        ...
                                        <div class="d-flex gap-4">
                                            ...
                                            <x-radio
                                                ...
                                                caption="Tidak"
                                                value="Tidak"
                                                :checked="($data_kuisioner[$question['name']] ?? '') === 'Tidak'"
                                            />
                                            <x-radio
                                                name="{{ $question['name'] }}"
...
<script>
    /* ═══════════════════════════════════════════════════════════════════
       VALIDASI KUISIONER: setiap pertanyaan wajib dipilih, tidak ada
       jawaban default. Kalau ada yang belum dipilih, tampilkan warning
       dan batalkan submit. Handler ini didaftarkan SEBELUM init_form()
       supaya jalan lebih dulu dan bisa menghentikan proses simpan.
    ═══════════════════════════════════════════════════════════════════ */
    $('#form_info').on('submit', function (e) {
        let unanswered = [];

        $('#tab_kuisioner [id^="qgroup_"]').removeClass('border border-danger rounded-2 p-2');

        @foreach($questions as $question)
            if (!$('input[name="{{ $question['name'] }}"]:checked').length) {
                unanswered.push(@json($question['label']));
                $('#qgroup_{{ $question['name'] }}').addClass('border border-danger rounded-2 p-2');
            }
        @endforeach
        ...
        if (unanswered.length > 0) {
            ...
            // pindah ke tab Kuisioner supaya user langsung melihat yang belum diisi
            $('a[href="#tab_kuisioner"]').tab('show');

            Swal.fire({
                icon: 'warning',
                title: 'Kuisioner Belum Lengkap',
                html: 'Mohon pilih jawaban untuk pertanyaan berikut:<br><br>' +
                      '<div class="text-start">' + unanswered.map(q => '• ' + q).join('<br>') + '</div>'
            });
    ...
    // hilangkan highlight merah begitu pertanyaan itu sudah dijawab
    $(document).on('change', '#tab_kuisioner input[type="radio"]', function () {
        $(this).closest('[id^="qgroup_"]').removeClass('border border-danger rounded-2 p-2');
    });
