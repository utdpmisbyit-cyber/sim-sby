...
@push('scripts')
    <script>
        ...
        let cek_umur_lalu_daftar = (donor_id, umur, forceDuplikat = false) => {
            if (umur !== null && umur !== undefined && (umur < USIA_MIN || umur > USIA_MAX)) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        proses_pendaftaran(donor_id, forceDuplikat, true); // forceUmur = true
                    } else {
                        Swal.fire('Dibatalkan', 'Pendaftaran donor tidak dilanjutkan karena batasan umur.', 'info');
                    }
                });
            } else {
                proses_pendaftaran(donor_id, forceDuplikat, false);
            }
        }

        // ═══════════════════════════════════════════════════════════════════
        // proses_pendaftaran — kirim flag force/force_umur,
        //   dan handle 409 dari backend sebagai fallback/safety net
        // ═══════════════════════════════════════════════════════════════════
        let proses_pendaftaran = (donor_id, forceDuplikat = false, forceUmur = false) => {
            $.post(base_url, {
                _token,
                donor_id,
                ...
                if (xhr.status === 409 && xhr.responseJSON) {
                    ...
                    if (jenis === 'sudah_daftar_hari_ini') {
                        Swal.fire({
                            ...
                            if (result.isConfirmed) proses_pendaftaran(donor_id, true, forceUmur);
                        });
                        ...
                        Swal.fire({
                            ...
                            if (result.isConfirmed) proses_pendaftaran(donor_id, forceDuplikat, true);
                        });
                    } else {
        ...
        // ═══════════════════════════════════════════════════════════════════
        // ★ FIXED: init_form (form tambah donor baru di modal pendaftaran)
            ...
            $form_info.submit((e) => {
                ...
                if (usia < USIA_MIN || usia > USIA_MAX) {
                    Swal.fire({
                        ...
                        text: `Usia pendonor ${usia} tahun (di luar rentang ${USIA_MIN}-${USIA_MAX} thn). Apakah pendonor tetap layak dan ingin melanjutkan pendaftaran?`,
                        icon: 'warning',
                        showCancelButton: true,
                        confirmButtonColor: '#3085d6',
                        cancelButtonColor: '#d33',
