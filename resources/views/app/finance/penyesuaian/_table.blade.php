<table class="table table-row-dashed table-hover fs-7 table-sm align-middle">
    ...
    <tbody>
        @forelse($penyesuaian as $item)
            <tr>
                <td>{{ ($penyesuaian->currentPage()-1)*$penyesuaian->perPage() + $loop->iteration }}</td>
                <td>{{ $item->kode }}</td>
                ...
                <!-- Nominal Debit -->
                <td>
                    {{ isset($item->nominal_debit) ? number_format($item->nominal_debit, 0, ',', '.') : '-' }}
                </td>

                <!-- Nominal Kredit -->
                <td>
                    {{ isset($item->nominal_kredit) ? number_format($item->nominal_kredit, 0, ',', '.') : '-' }}
                </td>
...
<div class="mt-3">{{ $penyesuaian->links() }}</div>