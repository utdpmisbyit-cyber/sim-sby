...
@section('title', 'Pengembalian Bank Darah Internal')
...
<div class="pk">
  ...
  <div class="pk-banner">
    <div>
      <h1>Pengembalian Bank Darah Internal</h1>
      <p>Manajemen pengembalian kantong darah antar Bank Darah / Unit Kerja internal</p>
    </div>
  ...
  <div class="pk-card">
    <div class="pk-card-head">
      <h5>Daftar Pengembalian Bank Darah Internal</h5>
    ...
    <div class="filter-bar">
      ...
      <input type="text"  id="fSearch" style="width:210px" placeholder="🔍 Cari nomor / unit…">
      ...
      <table class="pk-table">
        <thead>
          <tr>
            ...
            <th>Bank Darah Asal</th>
            <th>Bank Darah Tujuan</th>
        ...
        <tbody id="tableBody">
          <tr class="empty-row"><td colspan="10" style="text-align:center;padding:40px;color:var(--slate)">Memuat data…</td></tr>
        </tbody>
      </table>
    </div>
  ...
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="mfTitle">Tambah Pengembalian Bank Darah Internal</h5>
      ...
      <div class="modal-body">
        <div class="row g-3">
          ...
          {{-- Bank Darah Asal — AUTOCOMPLETE --}}
          <div class="col-md-6">
            <label class="flabel">Bank Darah Asal <span style="font-weight:400;text-transform:none">(yang mengembalikan)</span></label>
            <div class="ac-wrap">
              <input type="text"   id="fAsal"     class="finput" placeholder="Ketik nama / kode unit…" autocomplete="off">
              <input type="hidden" id="fAsalId">
              <div class="ac-drop" id="acAsal"></div>
            </div>
          </div>

          {{-- Bank Darah Tujuan — AUTOCOMPLETE --}}
          <div class="col-md-6">
            <label class="flabel">Bank Darah Tujuan <span style="font-weight:400;text-transform:none">(yang menerima kembali)</span></label>
            <div class="ac-wrap">
              <input type="text"   id="fTujuan"     class="finput" placeholder="Ketik nama / kode unit…" autocomplete="off">
              <input type="hidden" id="fTujuanId">
...
<div class="modal fade modal-pke" id="modalView" tabindex="-1">
  <div class="modal-dialog" style="max-width:900px">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title">Detail Pengembalian Bank Darah Internal</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
      </div>
      <div class="modal-body">
...
═══════════════════════════════════════════════════════ */
const R = {
  data         : '{{ route("penyimpanan.pengembalian_bank_darah_internal.data") }}',
  next         : '{{ route("penyimpanan.pengembalian_bank_darah_internal.nextNomor") }}',
  cari         : '{{ route("penyimpanan.pengembalian_bank_darah_internal.cariStok") }}',
  store        : '{{ route("penyimpanan.pengembalian_bank_darah_internal.store") }}',
  searchBank   : '{{ route("penyimpanan.pengembalian_bank_darah_internal.searchBankDarah") }}',
  searchPetugas: '{{ route("penyimpanan.pengembalian_bank_darah_internal.searchPetugas") }}',
  show         : id => `{{ url("/penyimpanan/pengembalian_bank_darah_internal") }}/${id}`,
  update       : id => `{{ url("/penyimpanan/pengembalian_bank_darah_internal") }}/${id}`,
  destroy      : id => `{{ url("/penyimpanan/pengembalian_bank_darah_internal") }}/${id}`,
};
...
async function loadTable() {
  ...
  tbody.innerHTML = `<tr class="empty-row"><td colspan="10" style="text-align:center;padding:36px">
  ...
  try {
    ...
    state.loading = false;
    tbody.innerHTML = `<tr class="empty-row"><td colspan="10" style="text-align:center;padding:36px;color:var(--danger)">
...
function renderTable(rows) {
  const tbody = document.getElementById('tableBody');
  if (!rows.length) {
    tbody.innerHTML = `<tr class="empty-row"><td colspan="10" style="text-align:center;padding:40px;color:var(--slate)">
      <div style="font-size:32px;margin-bottom:8px">📭</div>
      Tidak ada data pengembalian bank darah internal</td></tr>`;
    return;
  }
  ...
  tbody.innerHTML = rows.map((r, i) => `
    <tr>
      ...
      <td>${r.bank_darah_asal ?? '<span style="color:var(--slate)">—</span>'}</td>
      <td>${r.bank_darah_tujuan ?? '<span style="color:var(--slate)">—</span>'}</td>
...
═══════════════════════════════════════════════════════ */
let acAsal, acTujuan, acKembali, acTerima;

document.addEventListener('DOMContentLoaded', () => {
  acAsal = makeAutocomplete({
    inputId  : 'fAsal',
    dropId   : 'acAsal',
    hiddenId : 'fAsalId',
    url      : R.searchBank,
...
document.getElementById('btnTambah').addEventListener('click', async () => {
  editId = null; items = [];
  resetForm();
  document.getElementById('mfTitle').textContent = 'Tambah Pengembalian Bank Darah Internal';
...
async function openEdit(id) {
  editId = id; items = [];
  resetForm();
  document.getElementById('mfTitle').textContent = 'Edit Pengembalian Bank Darah Internal';
  ...
  acAsal?.setValue(d.bank_darah_asal?.nama ?? '', d.bank_darah_asal_id ?? '');
  acTujuan?.setValue(d.bank_darah_tujuan?.nama ?? '', d.bank_darah_tujuan_id ?? '');
...
document.getElementById('btnSimpan').addEventListener('click', async () => {
  ...
  const payload = {
    ...
    bank_darah_asal_id   : document.getElementById('fAsalId').value   || null,
    bank_darah_tujuan_id : document.getElementById('fTujuanId').value || null,
...
async function openView(id) {
  ...
  document.getElementById('vHeader').innerHTML = `
    ...
    <div class="col-md-6">${vRow('Bank Darah Asal', d.bank_darah_asal?.nama ?? '—')}</div>
    <div class="col-md-6">${vRow('Bank Darah Tujuan', d.bank_darah_tujuan?.nama ?? '—')}</div>
