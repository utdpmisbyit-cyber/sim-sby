<div class="table-responsive">
    <table class="table align-middle table-row-dashed table-hover fs-7 table-sm">
        ...
        <tbody>
        @php($no = 1)
        @if($jenis_periksa_serologis instanceof \Illuminate\Pagination\LengthAwarePaginator)
            @php($no = (($jenis_periksa_serologis->currentPage()-1) * $jenis_periksa_serologis->perPage()) + 1)
        @endif
        @foreach($jenis_periksa_serologis as $item)
            <tr>
                <td class="ps-4">{{ $no++ }}</td>
...
@if($jenis_periksa_serologis instanceof \Illuminate\Pagination\LengthAwarePaginator)
    {{ $jenis_periksa_serologis->links('vendor.pagination.custom') }}
@endif
