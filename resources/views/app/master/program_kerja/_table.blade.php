<div class="table-responsive">
    <table class="table align-middle table-row-dashed table-hover fs-7 table-sm">
        ...
        <tbody>
        @php($no = 1)
        @if($program_kerjas instanceof \Illuminate\Pagination\LengthAwarePaginator)
            @php($no = (($program_kerjas->currentPage()-1) * $program_kerjas->perPage()) + 1)
        @endif
        @foreach($program_kerjas as $item)
            <tr>
                <td class="ps-4">{{ $no++ }}</td>
...
@if($program_kerjas instanceof \Illuminate\Pagination\LengthAwarePaginator)
    {{ $program_kerjas->links('vendor.pagination.custom') }}
@endif
