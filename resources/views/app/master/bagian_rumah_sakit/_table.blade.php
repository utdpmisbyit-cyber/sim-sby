<div class="table-responsive">
    <table class="table align-middle table-row-dashed table-hover fs-7 table-sm">
        ...
        <tbody>
        @php($no = 1)
        @if($bagian_rumah_sakits instanceof \Illuminate\Pagination\LengthAwarePaginator)
            @php($no = (($bagian_rumah_sakits->currentPage()-1) * $bagian_rumah_sakits->perPage()) + 1)
        @endif
        @foreach($bagian_rumah_sakits as $item)
            <tr>
                <td class="ps-4">{{ $no++ }}</td>
...
@if($bagian_rumah_sakits instanceof \Illuminate\Pagination\LengthAwarePaginator)
    {{ $bagian_rumah_sakits->links('vendor.pagination.custom') }}
@endif
