<div class="table-responsive">
    <table class="table align-middle table-row-dashed table-hover fs-7 table-sm">
        ...
        <tbody>
        @php($no = 1)
        @if($mobil_units instanceof \Illuminate\Pagination\LengthAwarePaginator)
            @php($no = (($mobil_units->currentPage()-1) * $mobil_units->perPage()) + 1)
        @endif
        @foreach($mobil_units as $item)
            <tr>
                <td class="ps-4">{{ $no++ }}</td>
...
@if($mobil_units instanceof \Illuminate\Pagination\LengthAwarePaginator)
    {{ $mobil_units->links('vendor.pagination.custom') }}
@endif
