<div class="table-responsive">
    <table class="table align-middle table-row-dashed table-hover fs-7 table-sm">
        ...
        <tbody>
        @php($no = 1)
        @if($jenis_darahs instanceof \Illuminate\Pagination\LengthAwarePaginator)
            @php($no = (($jenis_darahs->currentPage()-1) * $jenis_darahs->perPage()) + 1)
        @endif
        @foreach($jenis_darahs as $item)
            <tr>
                <td class="ps-4">{{ $no++ }}</td>
                <td class="text-nowrap">{{ $item->kode }}</td>
                <td>{{ $item->nama }}</td>
                <td>{{ $item->nama_pendek }}</td>
                <td>{{ $item->umur_darah }}</td>
...
@if($jenis_darahs instanceof \Illuminate\Pagination\LengthAwarePaginator)
    {{ $jenis_darahs->links('vendor.pagination.custom') }}
@endif
