<div class="table-responsive">
    <table class="table align-middle table-row-dashed table-hover fs-7 table-sm">
        ...
        <tbody>
        @php($no = 1)
        @if($metode_serologis instanceof \Illuminate\Pagination\LengthAwarePaginator)
            @php($no = (($metode_serologis->currentPage()-1) * $metode_serologis->perPage()) + 1)
        @endif
        @foreach($metode_serologis as $item)
            <tr>
                <td class="ps-4">{{ $no++ }}</td>
...
@if($metode_serologis instanceof \Illuminate\Pagination\LengthAwarePaginator)
    {{ $metode_serologis->links('vendor.pagination.custom') }}
@endif
