<div class="table-responsive">
    <table class="table align-middle table-row-dashed table-hover fs-7 table-sm">
        ...
        <tbody>
        @php($no = 1)
        @if($kewarganegaraans instanceof \Illuminate\Pagination\LengthAwarePaginator)
            @php($no = (($kewarganegaraans->currentPage()-1) * $kewarganegaraans->perPage()) + 1)
        @endif
...
@if($kewarganegaraans instanceof \Illuminate\Pagination\LengthAwarePaginator)
    {{ $kewarganegaraans->links('vendor.pagination.custom') }}
@endif
