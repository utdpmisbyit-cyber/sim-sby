<div class="table-responsive">
    <table class="table align-middle table-row-dashed table-hover fs-7 table-sm">
        ...
        <tbody>
        @php($no = 1)
        @if($kelas_tujuan_darahs instanceof \Illuminate\Pagination\LengthAwarePaginator)
            @php($no = (($kelas_tujuan_darahs->currentPage()-1) * $kelas_tujuan_darahs->perPage()) + 1)
        @endif
        @foreach($kelas_tujuan_darahs as $item)
            <tr>
                <td class="ps-4">{{ $no++ }}</td>
...
@if($kelas_tujuan_darahs instanceof \Illuminate\Pagination\LengthAwarePaginator)
    {{ $kelas_tujuan_darahs->links('vendor.pagination.custom') }}
@endif
