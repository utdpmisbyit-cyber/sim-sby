<div class="table-responsive">
    <table class="table align-middle table-row-dashed table-hover fs-7 table-sm">
        ...
        <tbody>
        @php($no = 1)
        @if($kelompok_barangs instanceof \Illuminate\Pagination\LengthAwarePaginator)
            @php($no = (($kelompok_barangs->currentPage()-1) * $kelompok_barangs->perPage()) + 1)
        @endif
        @foreach($kelompok_barangs as $item)
            <tr>
                <td class="ps-4">{{ $no++ }}</td>
...
@if($kelompok_barangs instanceof \Illuminate\Pagination\LengthAwarePaginator)
    {{ $kelompok_barangs->links('vendor.pagination.custom') }}
@endif
