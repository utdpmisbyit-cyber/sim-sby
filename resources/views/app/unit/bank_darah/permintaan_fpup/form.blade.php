...
<div class="container-fluid px-4 py-4">
    ...
    <form id="form-fpup"
          method="POST"
          action="{{ $mode === 'create' ? route('unit.bank_darah.permintaan_fpup.store') : route('unit.bank_darah.permintaan_fpup.update', $fpup) }}">
        @csrf
        ...
        <div class="sec-card">
            ...
            <div class="sec-body">
                <div class="fg fg-4">
                    {{-- Kode RS: ketik kode → auto-fill nama RS --}}
                    <div class="fgroup">
                        <label>Kode RS</label>
                    ...
                    {{-- Nama RS: autocomplete → auto-fill kode RS + info lain --}}
                ...
                <div class="fg fg-3">
                    <div class="fgroup">
                        ...
                        {{-- hidden jika perlu disimpan --}}
                        {{-- <input type="hidden" name="kelompok_rs" id="kelompok_rs"> --}}
                    </div>
                    <div class="fgroup">
            ...
            <div class="sec-body">
                <div class="fg fg-4">
                    <div class="fgroup" style="grid-column:span 2;">
                        ...
                        <input type="text" name="nama_pasien" value="{{ old('nama_pasien', $fpup->nama_pasien ?? '') }}" required placeholder="Nama lengkap pasien">
                ...
                <div class="fg fg-4">
                    ...
                    <div class="fgroup">
                        <label>Alamat</label>
                        <input type="text" name="alamat" value="{{ old('alamat', $fpup->alamat ?? '') }}" placeholder="Alamat lengkap">
                    </div>
                </div>
        ...
        <div class="sec-card">
            ...
            <div class="sec-body">
                ...
                {{-- Kapan Transfusi: tampil hanya jika Ya --}}
                ...
                {{-- Gejala: tampil hanya jika Reaksi = Ya --}}
                ...
                {{-- Detail Serologi: tampil hanya jika Ya --}}
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    ════════════════════════════════════════════ */
    const URL_GEN_NO    = '{{ route("unit.bank_darah.permintaan_fpup.next-no-fpup") }}';
    const URL_SEARCH_RS = '{{ route("unit.bank_darah.permintaan_fpup.search-rs") }}';
    const URL_RS_KODE   = '{{ route("unit.bank_darah.permintaan_fpup.rs-by-kode") }}';
    const URL_DIAGNOSA  = '{{ route("unit.bank_darah.permintaan_fpup.diagnosa") }}';
    ...
    const JENIS_RS_OPTS  = @json($jenis_rs);
    const KATEGORI_RS_OPTS = @json($kategori_rs);

    /* Existing details untuk edit mode */
    ...
    if (btnGen) {
        ...
        btnGen.click(); // auto generate saat buka form create
    }
    ...
    /* ── Kode RS: ketik kode lalu blur → auto-fill nama RS ── */
    const inputKodeRs = document.getElementById('kode_rs');
    inputKodeRs.addEventListener('blur', async function () {
        const kode = this.value.trim();
        if (!kode || inputNamaRs.value) return; // jangan overwrite jika nama sudah ada
        try {
    ...
    function toggleKapanTransfusi() {
        ...
        // Jika Tidak, kosongkan & readonly agar tidak ikut submit (opsional)
    ...
    /* ── Pernah Serologi → Kapan / Dimana / Hasil ── */
    ...
    function renderDetailRows() {
        ...
        detailRows.forEach((row, i) => {
            // JNS_DARAH bisa array atau object
