...
function select_image(target, target_preview, limit_size = 1.5) {
    ...
    fileInput.onchange = function () {
        ...
        if (file) {
            ...
            const maxSize = limit_size * 1024 * 1024;
            if (file.size > maxSize) {
                swal.fire({
                    icon: 'warning',
                    title: 'File size must not exceed '+ limit_size.toString() +' MB.'
                });
                this.value = "";
                return;
            }
...
let remove_image = (target, target_preview, default_url) => {
