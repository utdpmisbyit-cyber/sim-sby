...
use App\Models\PengembalianDarahCrossmatchDetail;
use Illuminate\Support\Facades\DB;
use Illuminate\Support\Facades\Log;
use Illuminate\Support\Facades\Schema;
use Carbon\Carbon;

class PengembalianDarahCrossmatchService
{
    // ─── Utility ──────────────────────────────────────────────────────────────
    ...
    public function scanFpup(string $noFpup): array
    {
        try {
            ...
            // ══════════════════════════════════════════════════════════════════
            if (Schema::hasTable('pelayanan_crosstest')) {
                $cols     = Schema::getColumnListing('pelayanan_crosstest');
                $orderCol = in_array('created_at', $cols) ? 'created_at'
                    : (in_array('id', $cols) ? 'id' : $cols[0]);

                $row = DB::table('pelayanan_crosstest')
                ...
                if ($row) {
                    ...
                    $data['tgl_fpup'] = $this->dateOnly(
                        $this->pick($r, ['tgl_fpup','tanggal_fpup','tgl_permintaan','tgl_crossmatch','tgl_order'])
                    );
                    ...
                    Log::info('[scanFpup] TAHAP1 pelayanan_crosstest', [
                        'no_fpup'          => $noFpup,
            ...
            // ══════════════════════════════════════════════════════════════════
            // TAHAP 2 — pemberian_darah_crossmatch (lengkapi data yang null)
            // ══════════════════════════════════════════════════════════════════
            $tabelPemberian = $this->findTable([
                'pemberian_darah_crossmatch',
                'pemberian_darah',
                'pemberian_crossmatch',
                'pemberian',
            ]);
            ...
            if ($found) {
                ...
                if (Schema::hasTable('pengembalian_darah_crossmatch')) {
                    $existingList = DB::table('pengembalian_darah_crossmatch')
                        ->where('no_fpup', $noFpup)
    ...
    public function scanStock(string $noStock): array
    {
        try {
            ...
            if (Schema::hasTable('pengembalian_darah_crossmatch_detail')
                && Schema::hasTable('pengembalian_darah_crossmatch')) {

                $existingReturn = DB::table('pengembalian_darah_crossmatch_detail as d')
                    ->join('pengembalian_darah_crossmatch as p', 'p.id', '=', 'd.pengembalian_id')
    ...
    public function store(array $data): PengembalianDarahCrossmatch
    {
        return DB::transaction(function () use ($data) {
            $data['nomor_kembali'] = PengembalianDarahCrossmatch::generateNomor();

            $pengembalian = PengembalianDarahCrossmatch::create([
    ...
    public function update(PengembalianDarahCrossmatch $pengembalian, array $data): PengembalianDarahCrossmatch
    {
    ...
    public function destroy(PengembalianDarahCrossmatch $pengembalian): bool
    {
    ...
    {
        $query = PengembalianDarahCrossmatch::with('details')->orderByDesc('tanggal_kembali');
