...
class DonorController extends IoResourceController
{
    public function __construct()
    {
        ...
        $this->viewPrefix = 'app.mobil_unit.donor';
    ...
    /**
     * ★ GET /mobil_unit/donor/generate_kode
     * Dipanggil dari form_info.blade.php saat modal "Tambah Donor" dibuka,
     * untuk mengisi otomatis Kode & No. Pendaftaran (readonly) di form.
     * Sebelumnya method ini belum ada di controller meskipun route &
     * DonorService::generateKode() / generateNoPendaftaran() sudah siap,
     * sehingga memicu 500 (method tidak ditemukan) saat route diakses.
     */
    ...
    /**
     * ★ GET /mobil_unit/donor/get_donor_ke?no_ktp=...
     * Dipanggil dari form_info.blade.php saat No. KTP diketik (>=16 digit),
     * untuk auto-isi "Donor Ke-" beserta golongan darah & rhesus jika
     * No. KTP tersebut sudah pernah terdaftar sebelumnya.
     */
    public function getDonorKe(Request $request)
    {
        $request->validate([
        ...
        return response()->json(
            $this->service->getDonorKeByKtp($request->no_ktp)
        );
    }
