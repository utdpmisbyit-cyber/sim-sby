...
@section('title')
    Pendaftaran - apheresis Transfusi Darah -
@endsection
                ...
                <div class="d-flex flex-row justify-content-between align-items-center">
                    <div class="d-flex flex-column">
                        <h1 class="d-flex align-items-center my-1">
                            <span class="text-dark fw-bold fs-1">Pendaftaran Apheresis</span>
                        </h1>
...
@push('scripts')
    <script>
        ...
        let base_url = '{{ route('apheresis.pendaftaran.index') }}', donor_url = '{{ route('apheresis.donor.index') }}';
        let search_result = [];
    ...
    // 1. Ambil data donor terlebih dahulu untuk cek umur
    $.get(donor_url + '/' + donor_id, (donor) => {
        const umur = donor.umur; // Pastikan backend mengirimkan field 'umur'
        
        // 2. Logika Pengecekan Umur (< 17 atau > 60)
        if (umur < 17 || umur > 60) {
            Swal.fire({
                title: 'Perhatian: Batasan Umur',
                text: `Umur donor adalah ${umur} tahun. Donor di luar rentang 17-60 tahun biasanya memerlukan pemeriksaan ekstra. Apakah tetap layak dan ingin lanjut?`,
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    // Jika user klik Lanjut, jalankan pendaftaran
                    proses_pendaftaran(donor_id);
                } else {
                    // Jika tidak, tampilkan pesan dan biarkan form tetap terkunci/reset
                    Swal.fire('Dibatalkan', 'Pendaftaran donor tidak dilanjutkan karena batasan umur.', 'info');
                }
            });
        } else {
            // Jika umur normal (17-60), langsung proses
            proses_pendaftaran(donor_id);
        }
    }).fail((xhr) => {
...
// Pisahkan proses POST ke fungsi tersendiri agar kode lebih bersih
let proses_pendaftaran = (donor_id) => {
...
let cetak_form_pendaftaran = (data) => {
...
let kirim_wa = () => {
    ...
    let pesan = `*apheresis TRANSFUSI DARAH*\n\n` +
...
let cetak_tiket_from_row = (kode, nama, kode_donor, golongan, petugas) => {
    ...
    $form_info.submit((e) => {
        ...
        // 1. Ambil nilai usia dari input (berdasarkan gambar, input usia ada di form)
        // Pastikan selector name/id 'usia' sesuai dengan yang ada di komponen x-input Anda
        ...
        if (usia < 17 || usia > 60) {
            Swal.fire({
                ...
                text: `Usia pendonor ${usia} tahun (di luar rentang 17-60 thn). Apakah pendonor tetap layak dan ingin melanjutkan pendaftaran?`,
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#3085d6',
                cancelButtonColor: '#d33',
                ...
                if (result.isConfirmed) {
                    execute_save_donor($form_info);
                }
            });
        } else {
            // Jika usia normal, langsung simpan
            execute_save_donor($form_info);
        }
    });
}

// Fungsi pembantu untuk mengirim data ke server
let execute_save_donor = ($form) => {
    let url = donor_url;
    ...
    $.ajax({
        ...
        success: (result) => {
            init(); // Tutup modal
            select_donor(result.id); // Pilih donor dan cetak tiket
            Swal.fire('Berhasil', 'Data donor berhasil disimpan', 'success');
        }
    }).fail((xhr) => {
        // Jika ada error (misal validasi backend gagal), tampilkan di sini
