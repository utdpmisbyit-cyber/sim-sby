<div class="table-responsive">
    <table class="table align-middle table-row-dashed table-hover fs-7 table-sm">
        ...
        <tr class="text-start bg-secondary text-dark fw-bold fs-7 text-uppercase border-bottom-0">
            ...
            <th class="w-100px">No.Pengembalian</th>
            <th>Tanggal</th>
            <th>Bagian</th>
            <th>Petugas</th>
        ...
        <tbody>
        @php($no = 1)
        @if($pengembalian_kantongs instanceof \Illuminate\Pagination\LengthAwarePaginator)
            @php($no = (($pengembalian_kantongs->currentPage()-1) * $pengembalian_kantongs->perPage()) + 1)
        @endif
        @foreach($pengembalian_kantongs as $item)
            <tr>
                <td class="ps-4">{{ $no++ }}</td>
                ...
                <td class="text-end text-nowrap">
                    @if($item->deleted_at === null)
                        ...
                        <div class="menu menu-sub menu-sub-dropdown dropdown-menu menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-auto py-4" data-kt-menu="true">
                            <div class="menu-item px-3"><a onclick="confirm_perngembalian({{ $item->id }})" href="javascript:void(0)" class="menu-link px-3">Confirm Pengembalian</a></div>
                            @if($item->flag === 0)
...
@if($pengembalian_kantongs instanceof \Illuminate\Pagination\LengthAwarePaginator)
    {{ $pengembalian_kantongs->links('vendor.pagination.custom') }}
@endif
