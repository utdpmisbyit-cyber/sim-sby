                ...
                <div class="card card-flush border mb-4">
                    ...
                    <div class="card-body px-4 pt-3 pb-4">
                        <div class="row g-2">
                            ...
                            {{-- ASAL DARAH — custom search dropdown (TIDAK ada default UTD PMI Surabaya lagi, wajib dipilih manual) --}}
                            <div class="col-6">
                                <div class="fv-row">
                                    <label class="form-label fs-8 fw-bold text-muted mb-1">Tempat Donor <span class="required-mark">*</span></label>
                            ...
                            <div class="col-12">
                             <x-io-input :viewtype="2"
                                ...
                                :value="$donor->nama_asal_darah ?? ($donor->asalDarah->nama ?? '')"
                                readonly />
                            </div>
                        </div>
                    </div>
...
registerSearchDropdown({
    ...
    ajaxUrl    : '{{ route("mobil_unit.donor.select2.pekerjaan") }}',
});
...
registerSearchDropdown({
    ...
    ajaxUrl    : '{{ route("mobil_unit.donor.select2.kewarganegaraan") }}',
});
...
registerSearchDropdown({
    ...
    ajaxUrl    : '{{ route("mobil_unit.donor.select2.asal_darah") }}',
    onSelect   : function (item) {
        // auto-isi nama_asal_darah
        $('[name="nama_asal_darah"]').val(item ? item.text : '');
        $('#wrap_asal_darah').removeClass('is-invalid');
    },
});
...
registerSearchDropdown({
    ...
    ajaxUrl    : '{{ route("mobil_unit.donor.select2.wilayah") }}',
    onSelect   : function (item) {
...
registerSearchDropdown({
    ...
    ajaxUrl      : '{{ route("mobil_unit.donor.select2.kecamatan") }}',
...
/* ═══════════════════════════════════════════════════════════════════════════
   ...
   (kewarganegaraan, wilayah, kecamatan, pekerjaan, asal darah disimpan di
   hidden input, jadi atribut HTML5 "required" di kotak teksnya saja tidak
   cukup untuk memastikan user benar2 memilih item dari hasil pencarian)
═══════════════════════════════════════════════════════════════════════════ */
function validateRequiredDropdowns() {
    var requiredDropdowns = [
        ...
        { hidden: '#asal_darah_id',      wrap: '#wrap_asal_darah',      label: 'Tempat Donor' },
    ];
...
$('[name="no_fpup"]').on('change blur', function () {
    ...
    $.ajax({
        url  : '{{ route("mobil_unit.donor.check_fpup") }}',
        type : 'POST',
...
$('[name="no_ktp"]').on('input change', function () {
    ...
    _donor_ke_timer = setTimeout(function () {
        $.get('{{ route("mobil_unit.donor.get_donor_ke") }}', { no_ktp }, function (res) {
...
// pastikan Tempat Donor benar-benar kosong saat Tambah Donor baru
// (jaga-jaga kalau ada nilai lama/default yang ikut terbawa dari sisi server
// atau dari sisa data form sebelumnya saat modal dibuka berulang kali)
$('#asal_darah_id').val('');
$('#search_asal_darah').val('');
$('[name="nama_asal_darah"]').val('');
$('#wrap_asal_darah').removeClass('is-invalid');

$.get('{{ route('mobil_unit.donor.generate_kode') }}', function (res) {
