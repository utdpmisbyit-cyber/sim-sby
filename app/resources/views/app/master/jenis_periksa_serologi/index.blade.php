            ...
            <div class="card-body">
                <div class="d-flex flex-row justify-content-between align-items-center">
                    <div class="d-flex flex-column">
                        <h1 class="d-flex align-items-center my-1"><span class="text-dark fw-bold fs-1">Jenis Periksa Serologi</span></h1>
                        @include('layouts._breadcrumb')
                    </div>
                    <button type="button" onclick="info()" class="btn btn-sm btn-primary fw-bold border-0 fs-7">Tambah Jenis Periksa Serologi</button>
                </div>
...
@push('modals')
    <div class="modal fade modal-slide-right" tabindex="-1" id="modal_info">
        <div class="modal-dialog">
            <div class="modal-content" id="modal_info_item">
            </div>
        </div>
    </div>
@endpush
...
@push('scripts')
    <script>
        ...
        let selected_page = 1, _token = '{{ csrf_token() }}', base_url = '{{ route('master.jenis_periksa_serologi.index') }}', params_url = '{{ $params ?? '' }}';
