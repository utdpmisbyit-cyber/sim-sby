<div class="table-responsive">
    <table class="table align-middle table-row-dashed table-hover fs-7 table-sm">
        ...
        <tbody>
        @php($no = 1)
        @if($jenis_biayas instanceof \Illuminate\Pagination\LengthAwarePaginator)
            @php($no = (($jenis_biayas->currentPage()-1) * $jenis_biayas->perPage()) + 1)
        @endif
        @foreach($jenis_biayas as $item)
            <tr>
                <td class="ps-4">{{ $no++ }}</td>
...
@if($jenis_biayas instanceof \Illuminate\Pagination\LengthAwarePaginator)
    {{ $jenis_biayas->links('vendor.pagination.custom') }}
@endif
