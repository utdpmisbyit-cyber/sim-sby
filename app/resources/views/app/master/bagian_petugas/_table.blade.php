<div class="table-responsive">
    <table class="table align-middle table-row-dashed table-hover fs-7 table-sm">
        ...
        <tbody>
        @php($no = 1)
        @if($bagian_petugas instanceof \Illuminate\Pagination\LengthAwarePaginator)
            @php($no = (($bagian_petugas->currentPage()-1) * $bagian_petugas->perPage()) + 1)
        @endif
        @foreach($bagian_petugas as $item)
            <tr>
                <td class="ps-4">{{ $no++ }}</td>
...
@if($bagian_petugas instanceof \Illuminate\Pagination\LengthAwarePaginator)
    {{ $bagian_petugas->links('vendor.pagination.custom') }}
@endif
