<div class="table-responsive">
    <table class="table align-middle table-row-dashed table-hover fs-7 table-sm">
        ...
        <tbody>
        @php($no = 1)
        @if($reagen_serologis instanceof \Illuminate\Pagination\LengthAwarePaginator)
            @php($no = (($reagen_serologis->currentPage()-1) * $reagen_serologis->perPage()) + 1)
        @endif
        @foreach($reagen_serologis as $item)
            <tr>
                <td class="ps-4">{{ $no++ }}</td>
...
@if($reagen_serologis instanceof \Illuminate\Pagination\LengthAwarePaginator)
    {{ $reagen_serologis->links('vendor.pagination.custom') }}
@endif
