<div class="table-responsive">
    <table class="table align-middle table-row-dashed table-hover fs-7 table-sm">
        ...
        <tbody>
        @php($no = 1)
        @if($kelompok_rumah_sakits instanceof \Illuminate\Pagination\LengthAwarePaginator)
            @php($no = (($kelompok_rumah_sakits->currentPage()-1) * $kelompok_rumah_sakits->perPage()) + 1)
        @endif
        @foreach($kelompok_rumah_sakits as $item)
            <tr>
                <td class="ps-4">{{ $no++ }}</td>
...
@if($kelompok_rumah_sakits instanceof \Illuminate\Pagination\LengthAwarePaginator)
    {{ $kelompok_rumah_sakits->links('vendor.pagination.custom') }}
@endif
