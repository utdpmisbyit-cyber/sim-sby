<div class="table-responsive">
    <table class="table align-middle table-row-dashed table-hover fs-7 table-sm">
        ...
        <tr class="text-start bg-secondary text-dark fw-bold fs-7 text-uppercase border-bottom-0">
            ...
            <th>Nama</th>
            <th>Kelompok Rumah Sakit</th>
            <th class="text-center w-50px pe-4 rounded-end">Opsi</th>
        </tr>
        </thead>
        <tbody>
        @php($no = 1)
        @if($tujuan_darahs instanceof \Illuminate\Pagination\LengthAwarePaginator)
            @php($no = (($tujuan_darahs->currentPage()-1) * $tujuan_darahs->perPage()) + 1)
        @endif
        @foreach($tujuan_darahs as $item)
            <tr>
                <td class="ps-4">{{ $no++ }}</td>
                <td class="text-nowrap">{{ $item->kode }}</td>
                <td>{{ $item->nama }}</td>
                <td>{{ $item->kelompok_rumah_sakit->nama ?? '-' }}</td>
...
@if($tujuan_darahs instanceof \Illuminate\Pagination\LengthAwarePaginator)
    {{ $tujuan_darahs->links('vendor.pagination.custom') }}
@endif
