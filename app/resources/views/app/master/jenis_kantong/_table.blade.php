<div class="table-responsive">
    <table class="table align-middle table-row-dashed table-hover fs-7 table-sm">
        ...
        <tbody>
        @php($no = 1)
        @if($jenis_kantongs instanceof \Illuminate\Pagination\LengthAwarePaginator)
            @php($no = (($jenis_kantongs->currentPage()-1) * $jenis_kantongs->perPage()) + 1)
        @endif
        @foreach($jenis_kantongs as $item)
            <tr>
                <td class="ps-4">{{ $no++ }}</td>
                <td>{{ $item->nama }}</td>
...
@if($jenis_kantongs instanceof \Illuminate\Pagination\LengthAwarePaginator)
    {{ $jenis_kantongs->links('vendor.pagination.custom') }}
@endif
