<div class="table-responsive">
    <table class="table align-middle table-row-dashed table-hover fs-7 table-sm">
        ...
        <tbody>
        @php($no = 1)
        @if($service_costs instanceof \Illuminate\Pagination\LengthAwarePaginator)
            @php($no = (($service_costs->currentPage()-1) * $service_costs->perPage()) + 1)
        @endif
        @foreach($service_costs as $item)
            <tr>
                <td class="ps-4">{{ $no++ }}</td>
...
@if($service_costs instanceof \Illuminate\Pagination\LengthAwarePaginator)
    {{ $service_costs->links('vendor.pagination.custom') }}
@endif
