<div class="table-responsive">
    <table class="table align-middle table-row-dashed table-hover fs-7 table-sm">
        ...
        <tbody>
        @php($no = 1)
        @if($tipe_kantongs instanceof \Illuminate\Pagination\LengthAwarePaginator)
            @php($no = (($tipe_kantongs->currentPage()-1) * $tipe_kantongs->perPage()) + 1)
        @endif
        @foreach($tipe_kantongs as $item)
            <tr>
                <td class="ps-4">{{ $no++ }}</td>
...
@if($tipe_kantongs instanceof \Illuminate\Pagination\LengthAwarePaginator)
    {{ $tipe_kantongs->links('vendor.pagination.custom') }}
@endif
