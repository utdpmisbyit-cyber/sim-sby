<div class="table-responsive">
    <table class="table align-middle table-row-dashed table-hover fs-7 table-sm">
        ...
        <tbody>
        @php($no = 1)
        @if($hak_akses instanceof \Illuminate\Pagination\LengthAwarePaginator)
            @php($no = (($hak_akses->currentPage()-1) * $hak_akses->perPage()) + 1)
        @endif
        @foreach($hak_akses as $item)
            <tr>
                ...
                <td class="text-muted">
                    ...
                    {{ ucwords(strUnslug(join(', ', array_keys($description)), '_')) }}
                </td>
...
@if($hak_akses instanceof \Illuminate\Pagination\LengthAwarePaginator)
    {{ $hak_akses->links('vendor.pagination.custom') }}
@endif
