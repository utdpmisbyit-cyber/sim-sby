<div class="table-responsive">
    <table class="table align-middle table-row-dashed table-hover fs-7 table-sm">
        ...
        <tbody>
        @php($no = 1)
        @if($kelompok_biayas instanceof \Illuminate\Pagination\LengthAwarePaginator)
            @php($no = (($kelompok_biayas->currentPage()-1) * $kelompok_biayas->perPage()) + 1)
        @endif
        @foreach($kelompok_biayas as $item)
            <tr>
                <td class="ps-4">{{ $no++ }}</td>
...
@if($kelompok_biayas instanceof \Illuminate\Pagination\LengthAwarePaginator)
    {{ $kelompok_biayas->links('vendor.pagination.custom') }}
@endif
