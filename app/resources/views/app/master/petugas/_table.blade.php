<div class="table-responsive">
    <table class="table align-middle table-row-dashed table-hover fs-7 table-sm">
        ...
        <tbody>
        @php($no = 1)
        @if($petugas instanceof \Illuminate\Pagination\LengthAwarePaginator)
            @php($no = (($petugas->currentPage()-1) * $petugas->perPage()) + 1)
        @endif
        @foreach($petugas as $item)
            <tr>
                ...
                <td>{{ $item->hakAkses->hakAkses->nama ?? '-' }}</td>
                <td>{{ $item->no_telp }}</td>
...
@if($petugas instanceof \Illuminate\Pagination\LengthAwarePaginator)
    {{ $petugas->links('vendor.pagination.custom') }}
@endif
