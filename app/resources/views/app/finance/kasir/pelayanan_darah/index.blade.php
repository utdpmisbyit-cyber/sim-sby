...
<div class="bd-wrap">
    ...
    {{-- Filter --}}
    <div class="bd-filter">
        <form method="GET" action="{{ route('finance.pelayanan_darah.index') }}">
            <div>
                <label>Cari</label>
            ...
            <div style="display:flex;gap:6px;padding-top:18px">
                ...
                <a href="{{ route('finance.pelayanan_darah.index') }}" class="btn-f reset"><i class="fas fa-times"></i> Reset</a>
            </div>
        </form>
    </div>
                    ...
                    <td>
                        <div class="td-acts">
                            ...
                            @if($row->status === 'baru')
                                ...
                                <form method="POST" action="{{ route('finance.pelayanan_darah.update-status', $row) }}"
                            ...
                            @endif
                            @if($row->status !== 'lunas')
                                <form method="POST" action="{{ route('finance.pelayanan_darah.destroy', $row) }}"
...
<div class="bd-overlay" id="modal-create">
  <div class="bd-modal">
    ...
    <div class="bd-modal-body" style="max-height:75vh;overflow-y:auto">
        ...
        <form method="POST" action="{{ route('finance.pelayanan_darah.store') }}" id="form-create">
            @csrf
                        ...
                        <div class="bd-card-body">
                            <div class="bd-row2">
                                ...
                                <div class="bd-field">
                                    ...
                                    <input type="text" name="jenis_rs" id="c_jenis_rs" class="bd-input" value="{{ old('jenis_rs') }}" onchange="applyHargaOtomatis('c')">
                                </div>
                            </div>
                            <div class="bd-field" style="margin-top:10px">
                                <label>Nama RS</label>
                ...
                <div class="bd-col">
                    ...
                    <div class="bd-card">
                        ...
                        <div class="bd-card-body">
                            ...
                            <div class="bd-row2" style="margin-top:10px">
                                ...
                                <div class="bd-field">
                                    {{-- Jenis Biaya dari tabel jenis_biaya — onchange men-trigger lookup harga ulang --}}
                                    <label>Jenis Biaya</label>
                                    <select name="jns_biaya" id="c_jns_biaya" class="bd-input" onchange="applyHargaOtomatis('c')">
                    ...
                    <div class="bd-card">
                        <div class="bd-card-head">
                            ...
                            <button type="button" class="bd-btn bd-btn-red bd-btn-sm" onclick="addRow('c'); applyHargaOtomatis('c')">
...
<script>
// ── Route URLs ────────────────────────────────────────────────────
const URL_STORE       = "{{ route('finance.pelayanan_darah.store') }}";
const URL_SCAN        = "{{ route('finance.pelayanan_darah.scan-pemberian') }}";
const URL_NEXT_NO     = "{{ route('finance.pelayanan_darah.next-no-pelayanan') }}";
const URL_HARGA       = "{{ route('finance.pelayanan_darah.harga-satuan') }}";
const URL_BASE        = "{{ url('/') }}/{{ ltrim(parse_url(route('finance.pelayanan_darah.index'), PHP_URL_PATH), '/') }}";
 ...
 * - Harga satuan  → sudah dihitung otomatis di backend dari tabel service_costs
 *                   (cocokkan jns_darah + jenis_biaya), tidak perlu hitung ulang di JS.
 */
function doScan(pfx){
    ...
    fetch(URL_SCAN + '?q=' + encodeURIComponent(q))
        ...
        .then(d => {
            ...
            // ── Detail darah dari pemberian_darah_detail (harga sudah otomatis) ──
        ...
        <div class="bd-form-grid">
                ...
                <div class="bd-card">
                    ...
                    <div class="bd-card-body">
                        <div class="bd-row2">
                            ...
                            <div class="bd-field"><label>Jenis RS</label><input type="text" name="jenis_rs" id="e_jenis_rs" class="bd-input" value="${escHtml(d.jenis_rs||'')}" onchange="applyHargaOtomatis('e')"></div>
                        </div>
            ...
            <div class="bd-col">
                    ...
                    <div class="bd-card-body">
                        ...
                        <div class="bd-row2" style="margin-top:10px">
                            ...
                            <div class="bd-field"><label>Jenis Biaya</label>
                                <select name="jns_biaya" id="e_jns_biaya" class="bd-input" onchange="applyHargaOtomatis('e')">
                ...
                <div class="bd-card">
                    <div class="bd-card-head">
                        <span><i class="fas fa-tint"></i> Detail Darah</span>
                        <button type="button" class="bd-btn bd-btn-red bd-btn-sm" onclick="addRow('e'); applyHargaOtomatis('e')"><i class="fas fa-plus"></i> Baris</button>
                    </div>
                    <div class="bd-card-body" style="padding:0;overflow-x:auto">
                        <table class="bd-detail-table">
...
// ── Auto harga dari service_costs (Jenis RS × Jenis Biaya) ──
/**
 * Dipanggil saat "Jenis RS" atau "Jenis Biaya" berubah, atau saat baris baru
 * ditambahkan. Harga di tabel service_costs ditentukan oleh kombinasi
 * Jenis RS (Pemerintah/Swasta) × Jenis Biaya — SAMA untuk semua baris darah
 * dalam satu pelayanan, jadi cukup fetch sekali lalu terapkan ke semua baris.
 */
function applyHargaOtomatis(pfx){
    const jenisRs  = document.getElementById(pfx+'_jenis_rs')?.value || '';
    const jnsBiaya = document.getElementById(pfx+'_jns_biaya')?.value || '';
    if(!jnsBiaya) return; // tanpa jenis biaya, tidak ada acuan harga

    const params = new URLSearchParams({ jenis_rs: jenisRs, jns_biaya: jnsBiaya });
    fetch(URL_HARGA + '?' + params.toString())
        .then(r => r.json())
        .then(d => {
            if(!(d.harga_satuan > 0)) return;
            const tbody = document.getElementById('detail-body-'+pfx);
            if(!tbody) return;
            tbody.querySelectorAll('tr').forEach(tr => {
                const hargaInput = tr.querySelector('.row-harga');
                if(hargaInput){
                    hargaInput.value = d.harga_satuan;
                    hitungRow(hargaInput, pfx);
                }
            });
        })
        .catch(()=>{});
