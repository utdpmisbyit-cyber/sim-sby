...
@section('title', $title . ' Pemberian Darah')
...
@section('content')
{{-- px-2 agar tidak terlalu nempel di tepi layar --}}
...
<form id="formPemberian"
      method="POST"
      action="{{ $isEdit ? route('crossmatch.pemberian_darah.update', $pemberianDarah) : route('crossmatch.pemberian_darah.store') }}">
    @csrf
    ...
    <div class="card-main">
        ...
        <div class="card-title d-flex justify-content-between align-items-center">
            <span><i class="bi bi-droplet-fill me-2"></i>{{ strtoupper($title) }} PEMBERIAN DARAH CROSSMATCH</span>
            <a href="{{ route('crossmatch.pemberian_darah.index') }}" class="btn btn-sm btn-light py-0">
        ...
        <div class="p-2">
            ...
            {{-- ══ ROW 1: HEADER — semua kolom rata penuh ══ --}}
            ...
            <div class="section-fpup">
                ...
                {{-- Baris 1: No FPUP, Tgl FPUP, Dokter, Kode RS, Nama RS, Pasien — satu baris penuh --}}
                ...
                {{-- Baris 2: semua info detail RS dalam satu baris --}}
                <div class="row g-1">
                    <div class="col-xl-1 col-lg-2 col-md-2">
                    ...
                    {{-- No & Tgl Registrasi Online — masuk ke baris sama supaya hemat ruang --}}
                    <div class="col-xl-2 col-lg-2 col-md-3">
            ...
            {{-- ══ PILIH DARAH + PANEL KIRIM (rasio 8:4 supaya lebih lebar) ══ --}}
            <div class="row g-2 mb-row">

                {{-- ── Kiri: Tabel Pilih Jenis Darah ── --}}
                ...
                {{-- ── Kanan: Panel Jns Darah Yang Dikirim ── --}}
            ...
            <div class="d-flex align-items-center gap-3 mb-row">
                <div class="form-check form-check-inline mb-0">
                    <input class="form-check-input" type="checkbox" name="is_kandungan" id="isKandungan"
                           value="1" {{ $old('is_kandungan', $isEdit ? $pemberianDarah->is_kandungan : false) ? 'checked' : '' }}
                           {{ $isView ? 'disabled' : '' }}>
                    <label class="form-check-label" style="font-size:.78rem" for="isKandungan">Kandungan</label>
                </div>
                <div class="form-check form-check-inline mb-0">
                    <input class="form-check-input" type="checkbox" name="is_pasien_referral" id="isPasienReferral"
                           value="1" {{ $old('is_pasien_referral', $isEdit ? $pemberianDarah->is_pasien_referral : false) ? 'checked' : '' }}
                           {{ $isView ? 'disabled' : '' }}>
                    <label class="form-check-label" style="font-size:.78rem" for="isPasienReferral">Pasien Referral</label>
                </div>
            </div>

            {{-- ══ TABEL STOK DARAH — full width ══ --}}
            <div class="mb-row">
                    ...
                    <table class="table table-sm tbl-stok mb-0 w-100" id="tblStok">
                        <thead>
                            <tr>
                                ...
                                <th style="width:160px">Nostock</th>
                                <th style="width:100px">Jns Darah</th>
                                <th class="text-center" style="width:55px">Gol</th>
                                <th style="width:80px">Rh</th>
                                <th style="width:100px">Tgl Expired</th>
                                <th class="text-center" style="width:55px">CC</th>
                        ...
                        <tbody id="bodyStok">
                            @if(count($details) === 0)
                            <tr id="rowStokEmpty">
                                <td colspan="{{ $isView ? 8 : 9 }}" class="text-center text-muted py-3">
                            ...
                            @else
                                ...
                                <tr>
                                    ...
                                    <td class="text-center">{{ $d['jumlah'] ?? 1 }}</td>
                                    @if(!$isView)
                                    ...
                                    <input type="hidden" name="details[{{ $i }}][cc]"          value="{{ $d['cc'] ?? 0 }}">
                                    <input type="hidden" name="details[{{ $i }}][jumlah]"      value="{{ $d['jumlah'] ?? 1 }}">
                                </tr>
                                @endforeach
                            @endif
                        </tbody>
            ...
            {{-- ══ METODE / HASIL / KETERANGAN — satu baris penuh ══ --}}
            <div class="row g-1">
                <div class="col-xl-2 col-lg-2 col-md-3">
                    <label class="lbl-sm">Metode</label>
                    <select name="metode" class="form-select form-select-sm {{ $isView ? 'inp-ro' : '' }}"
                            {{ $isView ? 'disabled' : '' }}>
                        <option value="">-- Pilih --</option>
                        @foreach(['Gel Test', 'Tube Method', 'Slide Method', 'Lainnya'] as $m)
                            <option value="{{ $m }}" {{ $old('metode') === $m ? 'selected' : '' }}>{{ $m }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-xl-2 col-lg-2 col-md-3">
                    <label class="lbl-sm">Hasil</label>
                    <select name="hasil" class="form-select form-select-sm {{ $isView ? 'inp-ro' : '' }}"
                            {{ $isView ? 'disabled' : '' }}>
                        <option value="">-- Pilih --</option>
                        @foreach(['Sesuai','Tidak Sesuai','Compatible','Incompatible'] as $h)
                            <option value="{{ $h }}" {{ $old('hasil') === $h ? 'selected' : '' }}>{{ $h }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-xl-8 col-lg-8 col-md-6">
                    <label class="lbl-sm">Keterangan</label>
                    <input type="text" name="keterangan"
                           class="form-control form-control-sm {{ $isView ? 'inp-ro' : '' }}"
                           value="{{ $old('keterangan') }}" {{ $isView ? 'readonly' : '' }}>
                </div>
            </div>
        ...
        <div class="footer-bar d-flex justify-content-between align-items-center">
            <a href="{{ route('crossmatch.pemberian_darah.index') }}" class="btn btn-secondary btn-sm">
        ...
        <div class="footer-bar d-flex gap-2">
            ...
            <a href="{{ route('crossmatch.pemberian_darah.edit', $pemberianDarah) }}" class="btn btn-warning btn-sm">
...
const URL_SCAN_FPUP    = '{{ route('crossmatch.pemberian_darah.scan_fpup') }}';
const URL_SCAN_STOCK   = '{{ route('crossmatch.pemberian_darah.scan_stock') }}';
const URL_SCAN_PETUGAS = '{{ route('crossmatch.pemberian_darah.scan_petugas') }}';
...
function addStokRow(d) {
    ...
    tr.innerHTML = `
        ...
        <input type="hidden" name="details[${idx}][cc]"          value="${d.cc || 0}">
        <input type="hidden" name="details[${idx}][jumlah]"      value="1">
    `;
...
function removeStokRow(btn) {
    ...
    if (document.querySelectorAll('#bodyStok tr').length === 0) {
        document.getElementById('bodyStok').innerHTML =
            `<tr id="rowStokEmpty"><td colspan="9" class="text-center text-muted py-3">
