...
@section('title', 'Pelayanan Crossmatch — Hasil')
@section('page-title', 'Pelayanan Crossmatch — Hasil per Kantong')
...
<div class="row g-3">
    ...
    <div class="col-12" id="formSection" style="display:none;">
        ...
        <div class="ct-card mt-3">
            <div class="ct-card-header">
                ...
                <span id="labelFormTitle">Form Hasil Crossmatch</span>
            </div>
            <div class="ct-card-body">
                @include('app.crossmatch.pelayanan_crosstest.form')
            </div>
        </div>
    </div>
    ...
    <div class="col-12">
        <div class="ct-card">
            <div class="ct-card-header">
                <i class="bi bi-table"></i> Daftar Hasil Crossmatch
            ...
            <div class="ct-card-body">
                ...
                <form method="GET" action="{{ route('crossmatch.pelayanan_crosstest.index') }}"
                      class="row g-2 mb-4">
                    <div class="col-md-3">
                    ...
                    <div class="col-md-1">
                        <div class="d-flex gap-2">
                            ...
                            <a href="{{ route('crossmatch.pelayanan_crosstest.index') }}"
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td colspan="12" class="text-center py-5 text-muted">
                                    ...
                                    <p class="mt-2 mb-0">Belum ada data hasil crossmatch</p>
                                </td>
                            </tr>
                            @endforelse
                        </tbody>
...
═══════════════════════════════════════════════════════════════════ */
const ROUTES = {
    scanFpup    : "{{ route('crossmatch.pelayanan_crosstest.scan_fpup') }}",
    scanStock   : "{{ route('crossmatch.pelayanan_crosstest.scan_stock') }}",
    scanPetugas : "{{ route('crossmatch.pelayanan_crosstest.scan_petugas') }}",
    store       : "{{ route('crossmatch.pelayanan_crosstest.store') }}",
    base        : "{{ url('/crossmatch/pelayanan_crosstest') }}",
};
...
async function scanFpup(noFpup) {
    ...
    try {
        ...
        sv('hidPermintaanFpupId', res.permintaan_fpup_id);
...
/* ═══════════════════════════════════════════════════════════════════
   ...
   → Jika cocok → auto-set Hasil Crossmatch = "Cocok"
...
async function scanStock(noStock) {
    ...
    if (pendingRows.some(r => r.no_stock === noStock)) {
        toast(`No Stok ${noStock} sudah ada di daftar`, 'warning');
        sv('inpNoStock', '');
        return;
    }
    ...
    try {
        ...
        // ── Auto-fill identitas kantong ──────────────────────────
        sv('inpNoStock',  s.no_stock  || noStock);
        sv('inpJnsDarah', s.jns_darah || '');
        sv('inpGol',      s.gol       || '');
        sv('inpRhesus',   s.rhesus    || '+');

        // ── Auto-set Hasil Crossmatch jika cocok ─────────────────
        let hasilText = '';
        ...
        pendingRows.push({
            ...
            hasil     : res.is_compatible ? 'Cocok' : (fv('inpHasil') || ''),
            nat       : $('inpNat').checked,
        ...
        const msg = res.is_compatible
            ? `✓ ${s.no_stock} (${s.jns_darah||''} ${s.gol||''}${s.rhesus||''}) → COCOK`
            : `✓ ${s.no_stock} (${s.jns_darah||''} ${s.gol||''}${s.rhesus||''})${hasilText}`;
...
function resetForm() {
    ...
    $('labelFormTitle').textContent = 'Form Hasil Crossmatch';
