            ...
            <div class="card-body">
                <div class="d-flex flex-row justify-content-between align-items-center">
                    <div class="d-flex flex-column">
                        <h1 class="d-flex align-items-center my-1"><span class="text-dark fw-bold fs-1">Konfirmasi Litbang</span></h1>
                ...
                <form id="form_search" class="w-100 mt-4 bg-gray-200 border-solid border-2 border-gray-300 rounded-2 px-5 py-4">
                    ...
                    <div class="d-flex flex-lg-row flex-column align-items-lg-center justify-content-lg-between gap-4">
                        <div class="d-flex flex-row gap-6 flex-grow-1">
                            <x-input name="keyword" prefix="search_" caption="Cari No. Kantong / Nama / Kode Donor / Kode Aftap" />
                            <x-input name="tanggal" prefix="search_" caption="Tanggal Kirim" class="datepicker" />
                        </div>
...
@push('scripts')
    <script>
        ...
        let selected_page = 1, _token = '{{ csrf_token() }}', base_url = '{{ route('serologi.konfirmasi_litbang.index') }}', params_url = '{{ $params ?? '' }}';
        ...
        let info = (id = '') => { $.get(base_url + '/' + id + '/edit?' + params_url, (result) => display_modal_info(result)).fail((xhr) => display_modal_info(xhr.responseText)); }
        ...
        let init_form = (id = '') => {
            ...
            $form_info.submit((e) => {
                ...
                $.ajax({ url, type: 'post', data, cache: false, processData: false, contentType: false, success: (result) => {
                    if (result.errors) {
                        error_handle(JSON.stringify(result.errors));
                        return;
                    }
                    init();
