<div class="table-responsive">
    <table class="table align-middle table-row-dashed table-hover fs-7 table-sm">
        ...
        <tbody>
        @php($no = 1)
        @if($permintaan_suppliers instanceof \Illuminate\Pagination\LengthAwarePaginator)
            @php($no = (($permintaan_suppliers->currentPage()-1) * $permintaan_suppliers->perPage()) + 1)
        @endif
        @foreach($permintaan_suppliers as $item)
            <tr>
                <td class="ps-4">{{ $no++ }}</td>
                <td class="text-nowrap">{{ $item->no_permintaan ?? '' }}</td>
                <td class="text-nowrap">{{ formatDate($item->tgl_permintaan) }}</td>
...
@if($permintaan_suppliers instanceof \Illuminate\Pagination\LengthAwarePaginator)
    {{ $permintaan_suppliers->links('vendor.pagination.custom') }}
@endif
